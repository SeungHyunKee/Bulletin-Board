 ...
 * 						  Insert, Update, Select, Delete를 수행시킬수 있다
 * 
 * @Repository : Bean Container에 적재를 시키기위한 힌트.(데이터 접근 제어의 역할로 쓰임)
...
public class BoardDaoImpl extends SqlSessionDaoSupport implements BoardDao {
	...
	/*
	 * @Autowired : Bean Container / DI(Dependency Injection) 와 관련
	 * Bean Container : Spring Framework가 객체들을 생성해서 보관하는 '메모리 공간'
	 * 					@Controller, @Repository
	 * DI(Dependency Injection, 의존성 주입) : 
	 * -- Bean Container에 보관되어있는 객체를 필요한곳에 자동주입하는 기술을 말함
	 *-- 실행하는 방법
	 *   1. 생성자를 이용하는 방법
	 *   2. Setter를 이용하는 방법 -> @Autowired가 필요함
	 *   3. 멤버변수를 이용하는 방법
	 */
	@Autowired
	...
	public int getBoardAllCount() {
		//session: 서버와 클라이언트가 연결되어있는 정보를 말함
		// 서버 : database 서버
		// 클라이언트 : 스프링 애플리케이션
		// 이걸 sqlSession 이라고 하고, 이 sqlsession을 받아오는 명령어가 getSqlSession()
	...
	public int insertNewBoard(BoardVO boardVO) {
		/*
		 * MyBatis를 호출해서 쿼리가 실행될 수 있도록 하는 코드 작성.
